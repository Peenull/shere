...
import { User, Mail, Lock, Gift, ArrowRight, Loader } from 'react-feather';

import PhoneInput from '@/components/Form/PhoneInput';

// Reusable Input component with icon
const IconInput = ({ icon, label, ...props }: { icon: React.ReactNode; label: string; [key: string]: any }) => (
  <div>
...
export default function Signup() {
  ...
  const handlePhoneChange = (phone: string, country: string, currency: string) => {
  ...
  const phoneInputClasses = "bg-slate-800/50 border border-gray-700 text-gray-100 placeholder-gray-500 text-sm rounded-lg focus:ring-yellow-400 focus:border-yellow-400 block w-full p-3 transition-all duration-200";
      ...
      <div className="relative z-10 w-full max-w-lg">
        <div className="bg-slate-900/70 backdrop-blur-xl border border-gray-800/50 rounded-2xl p-8 shadow-2xl">
          <div className="text-center mb-8">
            <Link href="/" className="text-3xl font-black text-yellow-400 italic mb-4 inline-block">SHERE.</Link>
            <h1 className="text-2xl font-bold text-white">Create Your Account</h1>
            <p className="mt-2 text-gray-400 text-sm">Join the network and start earning today.</p>
          </div>
