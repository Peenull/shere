...
export default function HistoryPage() {
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    ...
    return (
      <span
        className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase border ${styles}`}
      >
        <Icon size={12} />
        {status}
      </span>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {activeTab === "withdrawals" ? (
              ...
              <div className="py-12 text-center bg-slate-900/30 rounded-2xl border border-white/5">
                <p className="text-slate-500">
                  No share purchase requests found.
                </p>
              </div>
            ) : (
              shares.map((s) => (
