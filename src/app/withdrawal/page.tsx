...
import { ArrowLeft, DollarSign, AlertCircle, CheckCircle, Smartphone } from 'react-feather';
import LoadingScreen from '@/components/LoadingScreen';

export default function WithdrawalPage() {
  const { user } = useAuth();
  const { balance, phone, phoneAccountName: existingAccountName, loading: userLoading } = useUserData();
  ...
  const handleRequestWithdrawal = async () => {
    ...
    if (withdrawAmount < 500) { // Example limit
      ...
      return (
        <div className="min-h-screen bg-slate-950 p-6 flex flex-col items-center justify-center text-center">
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl max-w-md w-full shadow-2xl">
                ...
                <p className="text-gray-400 mb-8">You need a balance to make a withdrawal. Start sharing your link to earn money!</p>
                <Link href="/" className="block w-full py-4 bg-yellow-400 text-black font-bold rounded-xl hover:bg-yellow-300 transition-colors">
    ...
    <div className="min-h-screen bg-slate-950 text-white font-sans p-4 md:p-8">
      <div className="max-w-lg mx-auto">
        <header className="mb-8 flex items-center gap-4">
            <Link href="/" className="p-2 bg-slate-900 rounded-lg hover:bg-slate-800 transition-colors">
        ...
        {status === 'success' ? (
            <div className="bg-slate-900 border border-green-500/30 p-8 rounded-2xl text-center shadow-2xl">
                ...
                <p className="text-gray-400 mb-8">Your withdrawal request for <span className="text-white font-bold">{parseFloat(amount).toLocaleString()} FCFA</span> has been received. Our team will review it shortly.</p>
                <Link href="/" className="block w-full py-4 bg-slate-800 text-white font-bold rounded-xl hover:bg-slate-700 transition-colors">
                    Return to Dashboard
                </Link>
            </div>
        ) : (
            <>
                <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 p-6 rounded-2xl mb-6 shadow-lg">
                    <p className="text-sm text-gray-500 font-medium uppercase tracking-wider mb-1">Available Balance</p>
                    <p className="text-4xl font-black text-white">{balance.toLocaleString()} <span className="text-yellow-400/80 text-xl">FCFA</span></p>
                </div>
                    ...
                    <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-lg">
                        <div className="flex items-center gap-3 mb-6">
                             ...
                             <div>
                                 ...
                                 <p className="text-sm text-gray-400">Add account name for {phone}</p>
                             </div>
                        </div>
                        
                        <div className="mb-6">
                            <label className="block text-sm font-medium text-gray-400 mb-2">Account Name (MOMO/OM)</label>
                            <input 
                                type="text"
                                value={accountName}
                            ...
                            <p className="text-xs text-gray-500 mt-2 flex items-start gap-1">
                                ...
                                This name must match your Mobile Money account. It cannot be changed later without admin support.
                            </p>
                        </div>
                    ...
                    <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-lg relative overflow-hidden">
                        {status === 'confirming' && (
                             <div className="absolute inset-0 bg-slate-900/95 z-10 flex flex-col items-center justify-center p-6 text-center backdrop-blur-sm animate-in fade-in zoom-in duration-200">
                                <h3 className="text-xl font-bold text-white mb-2">Confirm Request</h3>
                                <p className="text-gray-400 mb-6">Are you sure you want to withdraw <span className="text-white font-bold">{parseFloat(amount).toLocaleString()} FCFA</span> to <span className="text-white font-bold">{phone}</span> ({accountName})?</p>
                        ...
                        <div className="mb-6">
                            <label className="block text-sm font-medium text-gray-400 mb-2">Receiving Number</label>
                            <input 
                                type="text"
                                value={phone || ''}
                                disabled
                            ...
                            />
                             <div className="flex justify-between items-center mt-2 px-1">
                                <span className="text-xs text-gray-500">Account Name: {accountName}</span>
                            </div>
                        </div>

                        <div className="mb-8">
                            <label className="block text-sm font-medium text-gray-400 mb-2">Amount (FCFA)</label>
                            ...
                            <p className="text-xs text-gray-500 mt-2">Maximum withdrawal: {balance.toLocaleString()} FCFA</p>
                        </div>
