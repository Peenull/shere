...
import { Check, X, Users, PieChart, RefreshCw, AlertCircle, Smartphone } from 'react-feather';
import { approveWithdrawal, rejectWithdrawal, resetWithdrawalStatus } from '@/lib/firebase/withdrawalService';
...
export default function WithdrawalDetailModal({ withdrawal, onClose, onUpdate }: WithdrawalDetailModalProps) {
  ...
  const confirmApprove = async () => {
    ...
    setProcessing(true);
    try {
      await approveWithdrawal(withdrawal.userId, withdrawal.id, withdrawal.amount);
      onUpdate();
      onClose();
    } catch (e) {
      console.error(e);
        ...
        <div className="relative p-6 border-b border-slate-800/50 flex justify-between items-start z-10 shrink-0">
            <div>
                <h3 className="text-xl font-bold text-white mb-1 flex items-center gap-2">
                    {isConfirmingApprove && <AlertCircle className="text-blue-400" size={20}/>}
                    {isConfirmingApprove ? 'Confirm Approval' : 'Withdrawal Request'}
                </h3>
                <p className="text-sm text-gray-400 font-mono text-xs opacity-70">ID: {withdrawal.id}</p>
            </div>
            <button onClick={onClose} className="text-gray-500 hover:text-white transition-colors bg-slate-800/50 hover:bg-slate-700 p-2 rounded-full">
                <X size={18} />
            </button>
        </div>
        ...
        <div className="p-6 relative z-10 overflow-y-auto custom-scrollbar">
            ...
            {!isConfirmingApprove ? (
                ...
                <>
                    <div className="flex justify-between items-center mb-8 bg-slate-950/50 p-4 rounded-2xl border border-slate-800/50">
                        <div>
                            ...
                            <p className="text-3xl font-black text-white tracking-tight">{withdrawal.amount.toLocaleString()} <span className="text-yellow-400 text-lg font-normal">FCFA</span></p>
                        </div>
                        <div className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider shadow-sm ${
                            withdrawal.status === 'completed' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                        ...
                        {loadingUser ? (
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="p-4 bg-slate-900 rounded-xl border border-slate-800/80">
                                    ...
                                    <p className={`font-bold text-lg ${userData.balance < withdrawal.amount ? 'text-red-400' : 'text-green-400'}`}>
                                        {userData.balance?.toLocaleString()} <span className="text-xs text-gray-600">FCFA</span>
                                    </p>
                                </div>
                                <div className="p-4 bg-slate-900 rounded-xl border border-slate-800/80">
                                    <p className="text-xs text-gray-500 mb-1">Account Name (MOMO)</p>
                                    <p className="font-bold text-white truncate text-sm" title={userData.phoneAccountName}>
                                        {userData.phoneAccountName || 'Not Set'}
                                    </p>
                                </div>
                                 <div className="p-4 bg-slate-900 rounded-xl border border-slate-800/80 col-span-2">
                                    <p className="text-xs text-gray-500 mb-1">User Profile Name</p>
                            ...
                            <p className="text-red-400 text-xs bg-red-500/10 p-3 rounded-lg border border-red-500/20">Could not load user data.</p>
                        )}
                    </div>
                    
                    <div className="space-y-3 mb-8">
                        <div className="flex justify-between items-center p-4 bg-slate-800/30 rounded-xl border border-slate-800/30">
                            <span className="text-gray-400 text-sm flex items-center gap-2"><Smartphone size={14}/> Phone</span>
                            <span className="text-white font-mono font-medium tracking-wide">{withdrawal.phoneNumber}</span>
                        </div>
                        <div className="flex justify-between items-center p-4 bg-slate-800/30 rounded-xl border border-slate-800/30">
                             <span className="text-gray-400 text-sm flex items-center gap-2"><Users size={14}/> Name</span>
                            <span className="text-white font-medium">{withdrawal.phoneAccountName}</span>
                        </div>
                        {withdrawal.status === 'rejected' && (
                             <div className="p-4 bg-red-500/5 border border-red-500/10 rounded-xl">
                                <p className="text-xs text-red-500 font-bold mb-1 uppercase tracking-wider">Rejection Reason</p>
                                <p className="text-red-300 text-sm">{withdrawal.rejectionReason}</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-3">
                        {withdrawal.status === 'pending' && !isRejecting && (
                            <div className="flex gap-3">
                                ...
                                <button 
                                    ...
                                    disabled={processing || (userData && userData.balance < withdrawal.amount)}
                                    className="flex-[2] py-3.5 bg-gradient-to-r from-blue-600 to-blue-500 text-white font-bold rounded-xl hover:from-blue-500 hover:to-blue-400 transition-all shadow-lg shadow-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed text-sm flex items-center justify-center gap-2"
                                >
                                    Approve Payment
                                </button>
                            </div>
                        ...
                        {isRejecting && (
                            <div className="bg-slate-900 p-4 rounded-xl border border-slate-700 animate-in slide-in-from-bottom-4 fade-in duration-200">
                                <h4 className="text-sm font-bold text-white mb-3">Confirm Rejection</h4>
                                 <select 
                                    value={rejectReason}
                                ...
                                <div className="flex gap-3 mt-4">
                                     <button onClick={() => setIsRejecting(false)} className="flex-1 py-2 text-sm text-gray-400 hover:text-white transition-colors">Cancel</button>
                                     <button onClick={handleReject} disabled={processing} className="flex-1 py-2 bg-red-500/10 text-red-500 border border-red-500/20 text-sm font-bold rounded-lg hover:bg-red-500 hover:text-white transition-all">Confirm Reject</button>
                                </div>
                            </div>
                        )}
                ...
                <div className="text-center py-6 animate-in zoom-in-95 duration-200">
                    ...
                    <h4 className="text-xl font-bold text-white mb-2">Approve Payment?</h4>
                    <p className="text-gray-400 text-sm mb-8 px-6">
                        This will deduct <span className="text-white font-bold">{withdrawal.amount.toLocaleString()} FCFA</span> from the user's balance and mark the request as completed.
                    </p>
