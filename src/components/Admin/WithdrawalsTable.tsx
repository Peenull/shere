...
export default function WithdrawalsTable() {
  ...
  const fetchWithdrawals = async (reset = false) => {
    setLoading(true);
    try {
        const { withdrawals: data, lastDoc: last } = await getWithdrawals(activeTab, reset ? null : lastDoc);
  ...
  return (
    <>
        ...
        <div className="flex border-b border-slate-800 mb-6">
            {(['pending', 'completed', 'rejected'] as WithdrawalStatus[]).map(status => (
                <button
                    key={status}
                ...
                withdrawals.map((w) => (
                    ...
                    >
                         ...
                         <div className="relative z-10">
                            <h4 className="font-bold text-white text-lg mb-1 truncate tracking-tight">{w.phoneAccountName}</h4>
                         ...
                         <div className="mt-auto pt-4 border-t border-slate-800/50 flex justify-between items-end relative z-10">
                             <div className="flex flex-col">
                                <span className="text-[10px] uppercase text-gray-600 font-bold tracking-wider mb-0.5">Requested</span>
                                <span className="text-xs text-gray-400 font-mono">{formatDate(w.dateRequested).split(',')[0]}</span>
                             </div>
                             <p className="font-black text-white text-xl flex items-baseline">
                                 {w.amount.toLocaleString()} <span className="text-yellow-400 text-xs ml-1 font-bold">FCFA</span>
                             </p>
                         </div>
                    </div>
                ))
