...
const ViewUserModal: React.FC<ViewUserModalProps> = ({ isOpen, user, onClose, onInvest, onDelete }) => {
  ...
  const openExplorer = (user: UserData) => {
    setExplorerProps({ isOpen: true, user: user });
  };

  const closeExplorer = () => {
    setExplorerProps({ isOpen: false, user: null });
  };
  ...
  return (
    <>
        ...
        >
          ...
          <div className="flex-grow overflow-y-auto p-4 sm:p-6">
            ...
            <div className="bg-slate-900/50 rounded-lg p-4">
              <h3 className="font-bold text-white mb-3 flex items-center gap-2"><Users size={18}/> Invited Users ({user.invited?.length || 0})</h3>
      ...
      <UserExplorerModal
        isOpen={explorerProps.isOpen}
        user={explorerProps.user}
