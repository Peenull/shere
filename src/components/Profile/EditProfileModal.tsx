...
export default function EditProfileModal({ isOpen, onClose, currentName, currentPhone }: EditProfileModalProps) {
  const { user } = useAuth();
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-xl w-full max-w-md">
        ...
        <div className="p-4 bg-slate-950/50 flex justify-end gap-3 rounded-b-2xl">
          <button onClick={onClose} className="text-sm text-gray-300 hover:text-white transition px-4 py-2 rounded-md">
            Cancel
          </button>
          <button 
            onClick={handleSave}
