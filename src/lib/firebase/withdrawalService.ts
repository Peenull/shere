...
export const requestWithdrawal = async (
    ...
    try {
        ...
        const docRef = await addDoc(collection(db, `users/${userId}/withdrawals`), withdrawalData);
        return docRef.id;
    } catch (error) {
...
export const getWithdrawals = async (
    ...
    try {
        ...
        if (status === 'pending' || status === 'rejected') {
            constraints.push(orderBy('dateRequested', 'asc')); // First come first served
        ...
        const withdrawals: Withdrawal[] = snapshot.docs.map((doc: any) => ({
            id: doc.id,
            ...doc.data()
        } as Withdrawal));
